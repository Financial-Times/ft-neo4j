...
if [ "$1" == "neo4j" ]; then
    ...
    if [ "${GRAPHITE_ENABLED:-false}" = true ]; then
    	echo "metrics.enabled=${GRAPHITE_ENABLED}"
    	echo "metrics.graphite.enabled=${GRAPHITE_ENABLED:-true}"
    	echo "metrics.graphite.server=${GRAPHITE_ADDRESS}"
    	echo "metrics.graphite.interval=${GRAPHITE_INTERVAL:-3m}"
    	echo "metrics.prefix=${GRAPHITE_PREFIX}"
    fi
